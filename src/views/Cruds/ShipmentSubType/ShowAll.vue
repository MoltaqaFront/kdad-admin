    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Name Input -->
              <base-input col="4" type="text" :placeholder="$t('PLACEHOLDERS.Shipment_sub_type')"
                v-model.trim="filterOptions.title" />
              <!-- End:: Name Input -->

              <base-select-input col="4" :optionsList="allTypes" :placeholder="$t('PLACEHOLDERS.Shipment_main_type')"
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.Shipment_Sub_Types") }}</h5>
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.Shipment_main_type"),
          value: "shipmentTypeTitle.name",
          sortable: false,
          align: "center",
        },
        {
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/shipment-sub-type/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `modules/shipment-sub-type/${item.id}/activate`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/shipment-sub-type/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/shipment-sub-type/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `modules/shipment-sub-type/${this.itemToDelete.id}`,
        });
    ...
    async getAllCarType() {
      ...
      try {
        ...
        this.allTypes = res.data.extras.shipmentTypes;
