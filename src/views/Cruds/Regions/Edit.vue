  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `admin/states/${this.$route.params.id}`,
          data: REQUEST_DATA,
        });
        this.isWaitingRequest = false;
        this.$message.success(this.$t("MESSAGES.addedSuccessfully"));
        this.$router.push({ path: "/regions/all" });
      } catch (error) {
        this.isWaitingRequest = false;
    ...
    async getRegionsData() {
      try {
        ...
        this.data.nameAr = res.data.body.state.translations.title.ar;
        this.data.nameEn = res.data.body.state.translations.title.en;
