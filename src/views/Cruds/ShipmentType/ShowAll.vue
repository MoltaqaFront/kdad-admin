    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.Shipment_Types") }}</h5>
        ...
        <div class="title_route_wrapper" v-if="permissions.store">
          <router-link to="/shipment-type/create">
            {{ $t("PLACEHOLDERS.add_Shipment_type") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/shipment-type/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/shipment-type/${item.id}/activate`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/shipment-type/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/shipment-type/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `modules/shipment-type/${this.itemToDelete.id}`,
        });
