          ...
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- <base-select-input col="4" :optionsList="allStores" :placeholder="$t('PLACEHOLDERS.store')"
                v-model="filterOptions.storeType" /> -->
  ...
  methods: {
    ...
    async setTableRows() {
      ...
      try {
        let res = await this.$axios({
          method: "GET",
          url: "modules/coupon-client",
          params: {
            is_paginate: 1,
            page: this.paginations.current_page,
            search: this.filterOptions.name,
            store_id: this.filterOptions.storeType?.id,
            start_at: this.filterOptions.from_date,
            end_at: this.filterOptions.to_date,
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `modules/coupon-client/${this.itemToDelete.id}`,
        });
