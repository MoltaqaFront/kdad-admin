  ...
  methods: {
    ...
    async setTableRows() {
      ...
      try {
        let res = await this.$axios({
          method: "GET",
          url: "modules/financial-report?is_finished=1",
          params: {
    ...
    async confirmPdf() {
      try {
       let res = await this.$axios({
          method: "GET",
          url: `modules/financial-report?is_finished=1&export=pdf`,
        });
        this.pdf = res.data.data.pdf;
    ...
    async confirmExcel() {
      try {
       let res = await this.$axios({
          method: "GET",
          url: `modules/financial-report?is_finished=1&export=excel`,
        });
        this.excel = res.data.data.excel;
        // this.dialogPdf = false;
      } catch (error) {
