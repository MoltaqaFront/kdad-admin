    ...
    <main>
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <!-- End:: Title -->
          ...
          <!-- Start:: Title -->
          <template v-slot:[`item.title`]="{ item }">
          <p class="text-danger" v-if="!item.carTypeTitle"> {{ $t("TABLES.noData") }} </p>
          <p v-else>{{ item.title }}</p>
        </template>
        <!-- End:: Title -->
  ...
  methods: {
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            carTypeId: this.filterOptions.carType?.id,
          },
        });
        this.loading = false;
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `modules/car-category/${this.itemToDelete.id}`,
        });
