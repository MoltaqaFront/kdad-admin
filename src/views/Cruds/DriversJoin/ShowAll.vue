    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.provider_join") }}</h5>
...
export default {
  ...
  methods: {
    ...
    async HandlingItemActivationStatus(selectedItem) {
      ...
      if (this.itemToChangeActivationStatus.user.account_status == true) {
        this.status_now = 0
      } else {
        this.status_now = 1
      }
      const REQUEST_DATA = new FormData();
      REQUEST_DATA.append("deactive_reason", this.deactivateReason);
      ...
      try {
        ...
        await this.$axios({
          method: "POST",
          url: `modules/deactive-provider/${this.itemToChangeActivationStatus.user.id}?account_status=${this.status_now}`,
          data: REQUEST_DATA,
        });
    ...
    async confirmAcceptItem(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `modules/deactive-provider/${item.user.id}?account_status=1`
        });
