    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Phone Input -->
              <base-input col="3" type="tel" :placeholder="$t('TABLES.Clients.phone')"
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.id`]="{ item, index }">
          <span class="text-danger" v-if="!item.id"> {{ $t("TABLES.noData") }} </span>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <template v-if="!permissions.update && !permissions.delete && !permissions.activiate">
  ...
  methods: {
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `modules/admin/${this.itemToDelete.id}`,
        });
