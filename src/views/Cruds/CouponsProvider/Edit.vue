...
export default {
  ...
  methods: {
    ...
    // End:: Vuex Actions
      ...
      REQUEST_DATA.append("market_latitude", 0);
      REQUEST_DATA.append("market_longitude", 0);
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `modules/coupon-provider/${this.$route.params.id}`,
          data: REQUEST_DATA,
        });
    ...
    async getDataToEdit() {
      try {
        ...
        this.data.value_type = res.data.data.value_type;
        // End:: Set Data
      } catch (error) {
