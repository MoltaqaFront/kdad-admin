    ...
    <main>
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.slide`]="{ item }">
          <div class="table_image_wrapper">
            <h6 class="text-danger" v-if="!item.slide"> {{ $t("TABLES.noData") }} </h6>

            <button class="my-1" @click="showImageModal(item.slide)" v-else>
              <img class="rounded" :src="item.slide" :alt="item.name" width="60" height="60" />
            </button>
          </div>
        </template>
        <!-- End:: Item Image -->
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <a-tooltip placement="bottom" v-if="permissions.update" :class="{ 'disable_parent': item.can_edit === true }">
              <template slot="title">
  ...
  methods: {
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `modules/slide/${item.id}/activate`,
        });
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `modules/slide/${this.itemToDelete.id}`,
        });
