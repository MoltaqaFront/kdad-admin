        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              <!-- Start:: Name Input -->
              <base-input col="5" type="text" :placeholder="$t('TABLES.Faq.question')" v-model.trim="filterOptions.name" />
              <!-- End:: Name Input -->
              <!-- Start:: type list -->
              <!-- <base-select-input col="4" :optionsList="allTypes" :placeholder="$t('TABLES.Faq.type')"
                v-model="filterOptions.carType" required /> -->
              <!-- End:: type list -->
              <!-- Start:: Status Input -->
              <base-select-input col="5" :optionsList="activeStatuses" :placeholder="$t('PLACEHOLDERS.status')"
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.answer`]="{ item }">
          ...
          <div class="actions" v-else>
            <button class="btn_show" @click="showDescriptionModal(item.answer)">
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("TABLES.Faq.question"),
          value: "question",
          align: "center",
          sortable: false,
        },
        // {
        //   text: this.$t("TABLES.Faq.type"),
        //   value: "type.name",
        //   align: "center",
        //   sortable: false,
        // },
        {
          text: this.$t("TABLES.Faq.answer"),
          value: "answer",
          align: "center",
          sortable: false,
        },
  ...
  methods: {
    ...
    async getAllCarType() {
      ...
      try {
        ...
        this.allTypes = res.data.extras.types;
      } catch (error) {
        this.loading = false;
    ...
    showDescriptionModal(answer) {
      this.dialogDescription = true;
      this.selectedDescriptionTextToShow = answer;
    },
    // End:: Control Modals
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/main-categories/${item.id}/activate`,
        });
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `modules/faq/${this.itemToDelete.id}`,
        });
